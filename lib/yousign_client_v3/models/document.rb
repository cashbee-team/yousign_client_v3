  ...
  class Document
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `YousignClientV3::Document` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `YousignClientV3::Document`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'total_anchors')
        self.total_anchors = attributes[:'total_anchors']
      else
        self.total_anchors = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @total_anchors.nil?
        invalid_properties.push('invalid value for "total_anchors", total_anchors cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @total_pages < 1
      return false if @is_locked.nil?
      return false if @total_anchors.nil?
      true
    end
    ...
    def hash
      [id, filename, nature, content_type, sha256, is_protected, is_signed, created_at, total_pages, is_locked, initials, total_anchors].hash
    end
