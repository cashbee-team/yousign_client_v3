...
module YousignClientV3
  class UpdateWebhookSubscription
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `YousignClientV3::UpdateWebhookSubscription` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `YousignClientV3::UpdateWebhookSubscription`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@endpoint.nil? && @endpoint.to_s.length > 255
      ...
      if !@secret_key.nil? && @secret_key.to_s.length > 32
        invalid_properties.push('invalid value for "secret_key", the character length must be smaller than or equal to 32.')
      end

      if !@secret_key.nil? && @secret_key.to_s.length < 32
        invalid_properties.push('invalid value for "secret_key", the character length must be great than or equal to 32.')
      end
    ...
    def valid?
      ...
      return false if !@endpoint.nil? && @endpoint.to_s.length > 255
      return false if !@description.nil? && @description.to_s.length > 128
      return false if !@secret_key.nil? && @secret_key.to_s.length > 32
      return false if !@secret_key.nil? && @secret_key.to_s.length < 32
      true
    end
    ...
    def secret_key=(secret_key)
      if !secret_key.nil? && secret_key.to_s.length > 32
        fail ArgumentError, 'invalid value for "secret_key", the character length must be smaller than or equal to 32.'
      end

      if !secret_key.nil? && secret_key.to_s.length < 32
        fail ArgumentError, 'invalid value for "secret_key", the character length must be great than or equal to 32.'
      end
    ...
    def hash
      [endpoint, description, sandbox, subscribed_events, secret_key, scopes, workspaces, auto_retry, enabled].hash
    end
