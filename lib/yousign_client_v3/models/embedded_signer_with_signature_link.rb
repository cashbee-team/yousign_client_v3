  ...
  class EmbeddedSignerWithSignatureLink
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `YousignClientV3::EmbeddedSignerWithSignatureLink` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `YousignClientV3::EmbeddedSignerWithSignatureLink`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      status_validator = EnumAttributeValidator.new('String', ["initiated", "notified", "verified", "processing", "consent_given", "signed", "aborted", "error"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["initiated", "notified", "verified", "processing", "consent_given", "signed", "aborted", "error"])
    ...
    def hash
      [id, status, signature_link, signature_link_expiration_date].hash
    end
