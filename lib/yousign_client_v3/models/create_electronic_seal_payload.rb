  ...
  class CreateElectronicSealPayload
    ...
    def self.openapi_types
      {
        ...
        :'image_id' => :'String',
        :'external_id' => :'String',
        :'fields' => :'Array<CreateElectronicSealPayloadFieldsInner>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `YousignClientV3::CreateElectronicSealPayload` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `YousignClientV3::CreateElectronicSealPayload`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [document_id, image_id, external_id, fields, signature_level, certificate_id].hash
    end
