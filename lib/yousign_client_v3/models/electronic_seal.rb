  ...
  class ElectronicSeal
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `YousignClientV3::ElectronicSeal` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `YousignClientV3::ElectronicSeal`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      status_validator = EnumAttributeValidator.new('String', ["pending", "processing", "done", "error"])
      ...
      signature_level_validator = EnumAttributeValidator.new('String', ["electronic_signature", "advanced_electronic_signature"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["pending", "processing", "done", "error"])
    ...
    def signature_level=(signature_level)
      validator = EnumAttributeValidator.new('String', ["electronic_signature", "advanced_electronic_signature"])
    ...
    def hash
      [id, status, created_at, document_id, timestamp, image_id, external_id, signature_level].hash
    end
