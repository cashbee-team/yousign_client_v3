  ...
  class SignatureRequestInList
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `YousignClientV3::SignatureRequestInList` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `YousignClientV3::SignatureRequestInList`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, status, name, delivery_mode, created_at, ordered_signers, reminder_settings, timezone, email_custom_note, expiration_date, source, signers, approvers, labels, documents, sender, external_id, branding_id, custom_experience_id, signers_allowed_to_decline, workspace_id, audit_trail_locale, bulk_send_batch_id].hash
    end
