  ...
  class Text1
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `YousignClientV3::Text1` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `YousignClientV3::Text1`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@max_length.nil? && @max_length > 32767
      ...
      if !@question.nil? && @question.to_s.length > 255
    ...
    def valid?
      ...
      return false if !@max_length.nil? && @max_length > 32767
      return false if !@max_length.nil? && @max_length < 1
      return false if !@question.nil? && @question.to_s.length > 255
      true
    end
    ...
    def hash
      [signer_id, page, x, y, width, height, max_length, question, instruction, optional, font].hash
    end
