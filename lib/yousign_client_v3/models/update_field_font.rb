  ...
  class UpdateFieldFont
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `YousignClientV3::UpdateFieldFont` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `YousignClientV3::UpdateFieldFont`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      pattern = Regexp.new(/^#([a-f0-9]{6}|[A-F0-9]{6})$/)
      if @color !~ pattern
      ...
      if @size > 22
        invalid_properties.push('invalid value for "size", must be smaller than or equal to 22.')
      end
    ...
    def valid?
      ...
      return false if @color !~ Regexp.new(/^#([a-f0-9]{6}|[A-F0-9]{6})$/)
      return false if @size.nil?
      return false if @size > 22
      return false if @size < 8
    ...
    def size=(size)
      ...
      if size > 22
        fail ArgumentError, 'invalid value for "size", must be smaller than or equal to 22.'
      end
    ...
    def hash
      [family, color, size, variants].hash
    end
