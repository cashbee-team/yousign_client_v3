...
module YousignClientV3
  class CreateElectronicSealFieldReadOnlyTextPayload
    attr_accessor :type
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `YousignClientV3::CreateElectronicSealFieldReadOnlyTextPayload` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `YousignClientV3::CreateElectronicSealFieldReadOnlyTextPayload`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@text.nil? && @text.to_s.length > 80
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["read_only_text"])
      ...
      return false if @y < 0
      return false if !@text.nil? && @text.to_s.length > 80
      true
    end
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["read_only_text"])
    ...
    def hash
      [type, height, width, page, x, y, text].hash
    end
