  ...
  class SignatureRequestActivated
    ...
    def self.openapi_types
      {
        ...
        :'signers' => :'Array<EmbeddedSignerWithSignatureLink>',
        :'approvers' => :'Array<ApproverToNotify>',
        :'labels' => :'Array<SignatureRequestInListLabelsInner>',
        :'documents' => :'Array<SignatureRequestActivatedDocumentsInner>',
        :'external_id' => :'String',
        :'branding_id' => :'String',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `YousignClientV3::SignatureRequestActivated` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `YousignClientV3::SignatureRequestActivated`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @signers.length < 1
        invalid_properties.push('invalid value for "signers", number of items must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      status_validator = EnumAttributeValidator.new('String', ["ongoing", "approval"])
      ...
      return false if @signers.length < 1
      return false if @documents.nil?
      return false if @external_id.to_s.length > 255
      return false if @external_id !~ Regexp.new(/^[\w\-@.%+]+$/)
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["ongoing", "approval"])
    ...
    def signers=(signers)
      ...
      if signers.length < 1
        fail ArgumentError, 'invalid value for "signers", number of items must be greater than or equal to 1.'
      end
    ...
    def hash
      [id, status, name, delivery_mode, created_at, ordered_signers, reminder_settings, timezone, email_custom_note, expiration_date, signers, approvers, labels, documents, external_id, branding_id, custom_experience_id, audit_trail_locale].hash
    end
